...
console.log("=== INTEGRITY CHECK: FIREBASE IMPLEMENTATION (CJS) ===");
...
try {
    ...
    if (authContext.includes('signInWithPopup') && authContext.includes('firebase/auth')) {
        console.log("[PASS] AuthContext.jsx uses 'signInWithPopup' and imports 'firebase/auth'");
    } else {
        console.error("[FAIL] AuthContext.jsx DOES NOT use expected firebase auth functions");
        errors++;
    }
} catch (e) {
